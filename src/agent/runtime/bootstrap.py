...
def detect_weak_device(platform_info: dict, arch_info: dict) -> bool:
    ...
    try:
        ...
        pass
    mode = os.environ.get("GOAT_WEAK_DEVICE")
    if mode == "1":
        return True
    if mode == "0":
        return False
...
def build(configs_dir: str | None = None, repo_root: str | None = None):
    ...
    provider = os.environ.get("GOAT_PROVIDER") or default_cfg.get("provider", "auto")
    model = os.environ.get("GOAT_MODEL") or default_cfg.get("model")
    api_key = os.environ.get("GOAT_API_KEY") or default_cfg.get("api_key")
    base_url = os.environ.get("GOAT_BASE_URL") or default_cfg.get("base_url")
    llm = build_llm(
        provider=provider,
        model=model,
        api_key=api_key,
    ...
    agent = Agent(
        name=default_cfg.get("name", "goat-agent"),
        planner=planner,
        executor=executor,
        verifier=verifier,
