"""Unified LLM client factory. Picks a provider from config + environment.
...
def _env_for(provider: str) -> dict:
    if provider == "openai":
        return {"key": os.environ.get("OPENAI_API_KEY") or os.environ.get("UNIFIED_API_KEY")}
...
def _auto_detect(registry: ModelRegistry) -> str:
    for provider, env_key in {
        "openai": ("OPENAI_API_KEY", "UNIFIED_API_KEY"),
