...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="unified-agent",
        description="Unified cross-platform AI agent (Linux, Windows, Termux).",
    )
    ...
    p.add_argument("--api-key", help="API key (or set env UNIFIED_API_KEY).")
    ...
    p.add_argument("--version", action="version", version=f"unified-agent {__version__}")
    return p


def _apply_cli_overrides(args, agent):
    if args.provider:
        os.environ["UNIFIED_PROVIDER"] = args.provider
    if args.model:
        os.environ["UNIFIED_MODEL"] = args.model
    if args.api_key:
        os.environ["UNIFIED_API_KEY"] = args.api_key
    if args.base_url:
        os.environ["UNIFIED_BASE_URL"] = args.base_url
    if args.weak:
        os.environ["UNIFIED_WEAK_DEVICE"] = "1"
    return agent
