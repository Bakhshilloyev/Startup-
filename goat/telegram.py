...
class TelegramBot:
    ...
    def _handle(self, chat_id, text: str):
        ...
        if text.startswith("/model "):
            ...
            self.send(chat_id, f"model set to {self.agent.provider.model}")
            return
        if text.startswith("/provider "):
            from .llm import get_provider

            self.agent.provider = get_provider(self.agent.cfg)
            self.send(chat_id, f"provider set to {self.agent.provider.name}")
            return

        # Stream progress back to the chat as Goat works.
        def on_event(ev):
            if ev["type"] == "tool_call":
                self.send(chat_id, f"🔧 {ev['name']}")
    ...
    def _cmd_start(self, chat_id):
        self.send(
            ...
            "Commands: /help /reset /model /provider",
        )
    ...
    def _cmd_model(self, chat_id):
        self.send(
            chat_id,
            f"provider={self.agent.provider.name} model={self.agent.provider.model}",
        )
